...
# Baseline Core Role Taxonomy Matrix (Fallback)
CORE_ROLE_TAXONOMY = {
...
class GapAnalyzerADKAgent(ADKAgent):
    ...
    - Tier 1: BigQuery Live Data SQL Queries (if GCP BigQuery is enabled)
    - Tier 2: Static Industry Taxonomy Baseline
    ...
    def query_bigquery_live_data(self, target_role: str) -> Optional[List[str]]:
        """
        Executes a live SQL query against BigQuery Public Datasets / BigLake tables
        to pull real-time market-validated skill frequency weights.
        """
        ...
        try:
            ...
            # BigQuery SQL query across public job market dataset
            sql_query = """
                SELECT skill_name
            ...
            skills = [row.skill_name for row in results]
            return skills if skills else None
            
        except Exception as e:
            print(f"[BigQuery Query Note]: BigQuery client not initialized or project_id missing ({e}). Switching to Tier 2 taxonomy.")
            return None
    ...
    def analyze(self, candidate_skills: List[str], target_role: str) -> Dict[str, Any]:
        ...
        if not required_skills:
            ...
            try:
                ...
                print(f"[ADK 2.0 Gap Analyzer Error]: {e}")
